...
1. **TA-CN 集合 `extra = "allow"`**：`stock_basic_info` 等 Pydantic model 使用 `extra = "allow"`，允许存储额外字段。这意味着 unified_data adapter 可以安全地添加新的衍生字段（如 `ud_freshness_checked_at`）而不破坏 TA-CN 的读取。
2. **DSA 使用 SQLite**：DSA 的日线数据存储在 SQLite，与 TA-CN 的 MongoDB 不同。unified_data adapter 需要能只读兼容 DSA 既有 SQLite 数据；但 unified_data 自身不得新增 SQLite 持久化，所有新增缓存/审计/质量/DSA 强项数据集合统一写入 MongoDB `03_data_ud_*`。
...
5. **集合命名无前缀**：TA-CN 集合直接放在 `tradingagents` 库下，无模块前缀。unified_data 新增集合需要加 `03_data_ud_` 前缀隔离。
...
│  │  TA-CN MongoDB Adapter │ DSA SQLite Adapter │ Portfolio Adapter         │ │
│  │  (复用 stock_basic_info│ (复用 StockDaily)   │ (复用 IReader/IWriter)    │ │
│  │   stock_daily_quotes等)│                     │                           │ │
│  └────────┬──────────────────────────────────────────────────────────────┘ │
└───────────┼────────────────────────────────────────────────────────────────┘
            │
...
┌──────────────────────────────────────────────────────────────────────────┐
│                        Providers 数据源适配层                              │
│  TushareProvider │ AKShareProvider │ TA-CNAdapterProvider │ DSAAdapter     │
│  YFinanceProvider (P3) │ FinnhubProvider (P3)                             │
└───────────┬──────────────────────────────────────────────────────────────┘
            │
            ▼
┌──────────────────────────────────────────────────────────────────────────┐
│              外部数据源 / MongoDB 缓存                                     │
│  Tushare │ AKShare │ BaoStock │ yfinance │ MongoDB (tradingagents)       │
└──────────────────────────────────────────────────────────────────────────┘
```
...
│   ├── ta_cn_adapter.py               # TA-CNAdapterProvider
│   ├── dsa_adapter.py                 # DSAAdapterProvider
...
├── adapters/
│   ├── __init__.py
│   ├── ta_cn_mongo_adapter.py         # TA-CN MongoDB 集合读写
│   ├── dsa_sqlite_adapter.py          # DSA SQLite 读写
│   └── portfolio_adapter.py           # Portfolio IReader/IWriter 封装
│
...
| D03 | 实时行情 | 盘中决策、交易执行辅助 | ✅ market_quotes | ⚠️ StockQuote | Tushare/AKShare/腾讯/新浪 | ✅ 可复用 | adapter-only（读 TA-CN/DSA） | Phase 1 |
| D04 | 日线/周线/月线行情 | 技术分析、趋势判断、回测输入 | ✅ stock_daily_quotes | ✅ StockDaily (SQLite) | Tushare/AKShare/BaoStock/yfinance | ✅ 强复用 | reuse（adapter 读 TA-CN + DSA） | Phase 1 |
...
| DSA `StockDaily` (SQLite) | adapter → `DailyBar` canonical | `DSASQLiteAdapter.get_daily_bars()` | DSA | `market_data_service.get_kline_daily()`（兜底） |

**关键约束**：
- TA-CN adapter **只能读取**，不通过 unified_data 路径写入 TA-CN 集合。
- unified_data 自己的数据刷新走 Provider 路径（Tushare → AKShare），写入 `03_data_ud_cache_*` 缓存集合，与 TA-CN 主集合物理隔离。
- DSA adapter 只读 SQLite，不做写操作；SQLite 仅作为 DSA 既有数据源兼容层，不得成为 unified_data 新增数据的持久化后端。
...
| DSA `StockQuote` / `KLineData` | adapter 转换为 unified_data canonical | 作为兜底数据源使用 |
| DSA `DataFetcherManager` | 不直接复用其代码，参考其 fetcher 策略 | unified_data 独立实现 Provider 层 |
...
class DailyBar:
    ...
    @classmethod
    def from_dsa_stock_daily(cls, obj, security_id: SecurityId) -> "DailyBar": ...
```
...
#### TA-CNAdapterProvider

| 项 | 值 |
|---|---|
| name | `"ta_cn_adapter"` |
| markets | `{CN}` |
| capabilities | `market_data.realtime_quote`, `market_data.kline_daily`, `financial.*`, `metadata.stock_list`, `metadata.index_members`, `news.*` |
| is_available | MongoDB 中 TA-CN 数据可用（通过 collection 采样检测） |
| 实现策略 | 从 TA-CN MongoDB 集合读取已有数据，映射为 canonical objects |
...
| name | `"dsa_adapter"` |
| markets | `{CN, HK, US}` |
| capabilities | `market_data.kline_daily`, `market_data.realtime_quote` |
| is_available | DSA SQLite 数据库文件存在且可读 |
| 实现策略 | 从 DSA SQLite 读取 StockDaily，作为兜底数据源 |
...
fallback_chains:
  "market_data.kline_daily": ["tushare", "akshare", "ta_cn_adapter"]
  "market_data.realtime_quote": ["tushare", "akshare", "ta_cn_adapter"]
  ...
  "metadata.stock_list": ["tushare", "akshare", "ta_cn_adapter"]
  "metadata.index_members": ["tushare", "akshare"]
  "news.stock_news": ["tushare", "akshare", "ta_cn_adapter"]
  # Phase 3 新增
...
DataRouter.query()
    │
    ├─ CacheManager.get() ──→ 命中 + 未过期 ──→ 返回（freshness=cached）
    │
    ├─ 未命中 / 过期 / force_refresh
    │       │
    │       ▼
    │   Provider chain 执行
    │       │
    │       ├─ TushareProvider.fetch() ──→ 成功 ──→ CacheManager.put()
    │       ├─ AKShareProvider.fetch() ──→ 成功 ──→ CacheManager.put()
    │       └─ TA-CNAdapterProvider.fetch() ──→ 成功 ──→ CacheManager.put()
    │
...
- **缓存隔离**：`03_data_ud_cache_` 前缀确保与 TA-CN/DSA/portfolio 集合物理隔离。
...
**验收标准**：
- Fallback 链全部 provider 失败时抛出 AllProvidersFailedError（含摘要）
...
### Phase 7：TA-CN / DSA 渐进迁移规划（预计 1 天文档）
...
| `test_dsa_adapter` | 字段映射、SQLite fixture | 4 | 否（fixture） |
...
| `test_dsa_schema_compat` | 验证新 adapter 读取 DSA SQLite 不抛异常 | Phase 1 |
| `test_no_migration_regression` | 确认 TA-CN/DSA 集合未被修改（md5 hash 对比） | Phase 1 |
...
- `skills/research/daily_stock_analysis/data_provider/base.py` — DSA BaseFetcher
