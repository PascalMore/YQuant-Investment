...
当前 test_e2e_full_chain.py 约 993 行（≤1200 常规范围内），按场景拆分是维护性优化而非行数强制。T3 Remediation 可选用以下方案：
...
| `test_e2e_fixtures.py` | constants（KLINE_CAP 等）+ 全部 fixture 定义（`e2e_db`, `e2e_registry`, `e2e_ta_cn_miss`, …）+ `_make_db()` helper | ~260 | 不含 TestClass |
| `test_e2e_scene_1_2_3.py` | `TestE2EScene1_AllMissExternalSuccess` + `TestE2EScene2_CacheHit` + `TestE2EScene3_ProviderFallback` | ~230 | 按场景职责划分 |
| `test_e2e_scene_4.py` | `TestE2EScene4_AllFail` | ~110 | 按场景职责划分 |
| `test_e2e_scene_5.py` | `TestE2EScene5_ForceRefresh`（含 `_build_router` helper） | ~220 | 按场景职责划分 |
| `test_e2e_scene_6_7.py` | `TestE2EScene6_IndexDualPath` + `TestE2EScene7_CoverageGate` | ~180 | 按场景职责划分 |
...
| 编号 | 验收项 | 验证命令 | 期望 |
|---|---|---|---|
| A-001 | test_e2e_full_chain.py 存在 | `ls -la tests/data/unified_data/test_e2e_full_chain.py` | 文件存在 |
