...
| V0.2 | 2026-07-14 | Pascal 确认架构基线修订：共享 Mongo + internal-first 读取路径 + collection ownership 命名空间隔离 + DSA 不作为运行时数据源。更新 §4.1/§5.5/§7.4/§12。详见 Design 阶段修订说明。 | YQuant-Codex-Principal |
| V0.3 | 2026-07-14 | 文档同步修订（无代码/无生产副作用）：补充 §4.2 架构图、DSA 在 §7.4 作为分析/参考边界、§10.2 验收增加 internal-first / 不实现 DSA adapter / collection ownership 命名空间隔离三条硬约束；与 SPEC/DESIGN 一致保留 Pascal 架构基线。详见 §14 设计修订说明。 | YQuant-Principal |
...
### 架构基线修订（2026-07-14 Pascal 确认）

在 Design 阶段深入后发现原 RFC/SPEC 的读取路径设计存在硬冲突，Pascal 已确认以下架构基线修订，三层文档（RFC / SPEC / Design）均已同步：

1. **共享物理数据库**：Unified Data 与 TA-CN 共用同一物理 MongoDB `tradingagents`，不使用物理库隔离；逻辑 ownership 通过集合命名空间前缀实现。
2. **Internal-First 读取路径**：权威读取顺序为 TA-CN 既有数据 → UD 物化数据（`03_data_ud_*`）→ Query Cache（`03_data_ud_cache_*`）→ 外部 Provider。外部刷新失败不阻断已有内部数据读取。
3. **DSA 不是运行时数据源**：不实现 DSA SQLite / `StockDaily` adapter；DSA 仅在分析/参考中出现。
4. **三层语义分离**：TA-CN 既有业务资产（无前缀）/ UD 可追溯物化数据集（`03_data_ud_*`）/ 可丢弃短 TTL Query Cache（`03_data_ud_cache_*`）。
5. **Collection Ownership 禁止回写**：Unified Data 绝不回写、覆盖或在 TA-CN 既有无前缀集合中加字段污染。
...
**统一抽象 + Provider 可插拔 + Internal-First 读优先 + 渐进迁移**：

- 所有消费方通过统一接口访问数据，不直接对接底层 provider
- Provider 以 capability 声明自身能力，access layer 按 capability 路由
- **Internal-First 读取路径**：查询时先查共享 Mongo 内部源（TA-CN 既有数据 → UD 物化数据 → Query Cache），内部源全部未命中时再触发外部 Provider。外部刷新失败不阻断已有内部数据读取
...
- **MongoDB 优先 + 命名空间隔离**：缓存首选 MongoDB，与 TA-CN 既有集合共用同一物理库 `tradingagents`，通过集合命名空间前缀隔离（`03_data_ud_cache_*`）
- 每个数据域定义自己的缓存集合和 TTL
- Cache key 基于 `(security_id, domain, params)` hash
- 缓存命中时仍检查 freshness 策略
- 支持手动强制刷新（bypass cache）
- **三层语义分离**：TA-CN 既有资产（无前缀）/ UD 可追溯物化数据（`03_data_ud_*`）/ 可丢弃短 TTL Query Cache（`03_data_ud_cache_*`）
...
| 阶段 3（后续） | DSA **仅作为分析/参考**上下文出现在文档与回测参考中；**不实现 DSA adapter、不作为运行时数据源、不作为外部 fallback 链一员** | 低 |
| 阶段 4（远期） | TA-CN 内部逐步替换为直接调用 unified_data | 高（需逐模块验证） |

**迁移原则**：先并存后替换，每一步可回滚。DSA 不是运行时数据源；**不实现 DSA SQLite / `StockDaily` adapter**，DSA 不出现在 `external_fallback_chains` 中。
...
- [ ] RFC 明确以下 Pascal 架构基线（V0.3 同步）：① Unified Data 与 TA-CN 共用同一物理 MongoDB / `tradingagents`，不依赖物理库隔离；② 权威读取路径为 internal-first（共享 Mongo 内部源 → 外部 Provider）；③ 不实现 DSA SQLite / `StockDaily` adapter，DSA 不出现在 `external_fallback_chains` 中；④ TA-CN 既有无前缀集合仅允许只读复用，禁止回写/覆盖/加字段污染
...
3. ~~**缓存集合命名**：unified_data 的缓存集合是否加前缀（如 `03_data_03_data_ud_cache_*`）以区分 portfolio 集合？~~ **已解决（2026-07-14）**：Query Cache = `03_data_ud_cache_*`，物化数据 = `03_data_ud_*`，与 TA-CN 既有集合共用同一物理库 `tradingagents` 通过命名空间隔离。
4. **Provider 凭据统一**：Tushare token 当前分散在多处 `.env`，是否在 unified_data 层强制统一？
5. ~~**SecurityId 持久化**：SecurityId 转换映射是否需要持久化到 MongoDB（如 `security_master` 集合），还是纯内存计算？~~ **已解决**：纯内存计算，不持久化。
...
- DSA 数据源：`skills/research/daily_stock_analysis/data_provider/`（仅供分析/参考）
- data-pipeline：`skills/data/data-pipeline/SKILL.md`
- Pipeline skill：`skills/infra/ai-coding-pipeline/SKILL.md`

---

## 14. 文档同步修订说明（V0.3，2026-07-14 Pascal 架构基线同步）

本次为文档同步修订，无代码改动、无生产副作用。修订内容仅限于 RFC/SPEC/Design 三层文档的措辞统一与已确认基线在三处的明示落点。三层文档遵循原则：**先并存后替换、每步可回滚**；**DSA 不是运行时数据源**；**internal-first 永远是权威读取路径**。

**本次修订必须保持的不变量**：

1. **共享物理数据库**：Unified Data 与 TA-CN 共用 `tradingagents`；不引入物理库隔离，靠集合命名空间前缀（TA-CN 无前缀 / `03_data_ud_*` 物化 / `03_data_ud_cache_*` QueryCache / `10_infra_tc_*` TaskCenter）实现所有权。
2. **Internal-First 读取路径**：TA-CN 既有 → UD 物化 → Query Cache → 外部 Provider；外部刷新失败不阻断内部已有数据读取，必须返回明确的 `DataResult` 缺失/错误语义。
3. **DSA 边界**：DSA 仅作为分析/参考；不实现 DSA SQLite / `StockDaily` adapter；DSA 不出现在 `external_fallback_chains` 中。
4. **Collection Ownership 不可回写**：Unified Data 绝不回写、覆盖或加字段污染 TA-CN 既有无前缀集合。
5. **Task Center 先行**：最小 Task/Job/Execution、幂等、重试、执行审计能力在 UD 物化写入前须可用；不创建真实 Job、不启用 cron/systemd、长期调度。
