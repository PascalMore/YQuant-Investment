...
## 13. Phase 1D Closeout 与 Production Readiness 边界

### 13.1 阶段结果概要

Phase 1D 已完成全部实现与验证流程：

| 里程碑 | 状态 | 证据 |
|---|---|---|
| T5 Review（`t_976ad8a2`） | PASS | Kanban review 通过 |
| 独立离线 Verify（`t_9743d0b2`） | PASS | 48/48 Phase 1D 专项测试，零网络调用，基于 Kanban handoff + repo 静态证据 |
| 真实 Tushare smoke（`t_01a2457f`） | 已完成 | 见 §13.2 |
| 本 closeout 记录 | 2026-07-20 | — |

### 13.2 真实 smoke 验证结论

验证范围严格限定为 `CN + market_data.kline_daily` 的**单标的、5 自然日、Tushare 成功主路径**：

- **标的**：`600519`
- **日期**：`2026-07-13` ~ `2026-07-17`
- **Provider**：Tushare（`provider="tushare"` forced 路径）；AKShare 未触发
- **结果**：返回 5 条非空 `DailyBar`，日期与 OHLC 合理
- **调用统计**：Tushare fetch=1, AKShare=0, exit=0, 耗时 1.404s
- **副作用**：未触发 Mongo/cache/Audit/QualitySummary/调度/交易；repo 无 diff
- **凭据**：token 来自环境变量，未出现在任何文档或 Kanban 输出中

### 13.3 风险重评估（对照 RFC §7）

与原始风险登记表对比，closeout 阶段的风险状态更新：

| RFC §7 风险项 | 原始概率 | 当前评估 | 说明 |
|---|---|---|---|
| Tushare/AKShare API 漂移 | 中 | 未改变 | 单次 smoke 不足以降低该风险；API 版本锁定仍需持续监控 |
| 跨源单位不一致 | 高 | 未改变 | 仍在 OQ-2，本阶段未归一化 |
| 真实网络 smoke 不稳定 | 中 | 单次稳定 | 本次 1.404s 返回正常；单次不能排除 flaky，需更多采样 |
| token 泄露 | 低 | 未复现 | smoke 全程无泄露证据；需长期合规审查 |
| Router L809 空 list 误判 | 中 | **已解决** | Design §3.6 raise 方案经离线 48 项测试验证 |
| datetime.utcnow() 技术债 | 低 | 未改变 | 独立技术债卡（OQ-3） |

### 13.4 未验证项目（确认非本阶段目标）

以下项目**不在**本验证范围之内，且不构成 Phase 1D 闭坑阻塞：

- **AKShare 真实 fallback 路径**：smoke 中 Tushare 成功，AKShare 未触发。AKShare 的真实网络行为（列名、单位、日期格式、限流行为）仅由离线 fake 客户端测试覆盖。
- **配额/headroom 行为**：未做压力测试。单次 smoke 不建立、不证明 quota/headroom 基线或长期配额行为，且真实 API 调用会消耗 Tushare 配额。Tushare 200 RPM 限流框架已就位但未在真实配额限制下验证。
- **长期稳定性**：单次调用不能推断 24h/7d 连续运行行为。
- **其余 Tushare 12 条 + AKShare 6 条 capability 的真实返回**：均保持 stub，无真实网络验证。
- **RateLimiter 真实配额限制行为**：离线 fake 客户端覆盖逻辑路径，真实 Tushare API 的配额耗尽恢复行为未验证。
- **跨源单位归一化**：OQ-2 未启动，属后续阶段。

### 13.5 后续阶段建议

1. **真实 smoke 扩展**：建议在 Phase 2 前覆盖多标的、多日期范围（含无数据日）、AKShare fallback 路径的真实验证。
2. **额度监控**：Tushare 配额有限，连续性真实验证前需确认剩余配额并监控消耗。
3. **消费方集成**：当前 smoke 仅验证 Provider 层，建议在 strategies/portfolio/reports 等消费方中做端到端集成验证。
