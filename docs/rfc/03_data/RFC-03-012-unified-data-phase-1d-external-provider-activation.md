...
| 依赖 SPEC | SPEC-03-008（Phase 1B-A 查询平面）、SPEC-03-009（Phase 1B-B 持久化缓存平面） |
| 关联 Design | DESIGN-03-007 / DESIGN-03-008 / DESIGN-03-009（Phase 1B 设计） |
...
- **外部 Provider 仍是 stub**：`skills/data/unified_data/providers/tushare.py` 与 `akshare.py` 的 `fetch()` 调用 `_stub_columns.stub_dataframe_for(capability)` 返回**空 DataFrame**（仅 schema 列，无行）。`_to_canonical()` 是 `base_external.py` 的 no-op 透传。模块 docstring 把「真实 API 调用的激活」标注为「Phase 1B-B」，但 1B-B 已被 RFC-03-009 占用为持久化缓存平面，命名衔接需要在本阶段显式澄清（见 §2.3）。
- **环境尚未安装 `tushare` / `akshare`**：`is_available()` 会因 import 失败返回 `False`，这是 Phase 1B-A 的预期行为。激活前需确认运行环境具备这两个包与 `TUSHARE_TOKEN`。
...
为避免与 1B-B 持久化平面冲突，本阶段正式定义为 **Phase 1D: External Provider Activation**，紧接 1C 之后、Phase 2 之前/并行。stub docstring 中残留的「Phase 1B-B」字样属于历史前瞻标注，不构成事实契约；本 RFC 不回头修订 1B-A 已交付的 docstring（避免触碰已通过 Review 的代码），但在 §10 开放问题中记录「未来可在文档维护 pass 中同步措辞」。
...
- [ ] **激活 `market_data.kline_daily`（CN only）的真实网络调用**：TushareProvider 主源、AKShareProvider 兜底。其余 12 条 Tushare capability 与 6 条 AKShare capability **保持 stub 不变**。
- [ ] **固化字段映射契约**：Tushare / AKShare 原始列 → `DailyBar` canonical 的精确映射表，含日期格式、单位、空值、复权口径处理。
- [ ] **保持 internal-first 优先级与 forced-provider 语义**：`kline_daily` 的真实外部调用只在 Step 4（外部 fallback）或 `provider="tushare"` / `provider="akshare"` forced 分支触发；TA-CN 只读优先级、`force_refresh` 跳过内部源的语义不变。
- [ ] **确定性行为**：输入（`SecurityId`、日期范围、`limit`）、输出（`DataResult` 携带 `list[DailyBar]` 或 error）、日期格式、OHLCV/amount 单位、空结果、字段缺失、超时、限流、重试、可用性判定、双源全失败的终态行为全部可执行、可测试。
- [ ] **`DataResult` 兼容性**：`provider` / `source_trace` / `warnings` / `quality_score` 字段在真实调用路径下与 Phase 1B-A / Phase 2 契约一致；**不得制造或静默返回 stub/假数据**。
- [ ] **凭据与 token 安全**：Tushare token 只经环境变量/可注入客户端处理；**不得读取、回显、记录**真实秘密（P-10）。
- [ ] **测试分层**：fake HTTP / 可注入客户端单测、Router fallback 测试、数据合理性断言；真实网络 smoke 是后续受控验证项。

### 3.2 非目标（Out of Scope）

- [ ] **不加周线/月线、复权因子、财务三表、新闻、交易日历、估值、指数成分、实时行情的真实激活**（保持 stub）。
- [ ] **不迁移消费方**：不修改 strategies / portfolio / reports / stock framework 的调用代码；消费方迁移属后续阶段。
- [ ] **不实现 Sector Router**（DESIGN-03-010 已明确不在范围）。
- [ ] **不实现 Task Center 集成、批量回填、cron/systemd/webhook**。
- [ ] **不做生产副作用**：本阶段严禁 MongoDB DDL、真实 MongoDB 写入、materialization/cache 写入、AuditLogger 真实写入、QualitySummary 启用。
- [ ] **不修改 Phase 0 公共契约**：`SecurityId` / `DataResult` / `Market` / `Capability` 值对象的签名与语义不变；`DataResult.success` / `DataResult.error` 工厂语义不变。
...
- [ ] **不把真实网络 smoke 作为本卡或实现卡的执行前提**（见 §9.3）。
...
| **HTTP 客户端抽象** | 新增轻量客户端接口（构造注入或工厂），封装 Tushare/AKShare 的真实 SDK 调用；fake 实现用于单测 | 1B-A 未有，1D 新增 |
...
Router Step 4 解析 kline_daily 链 → ["tushare", "akshare"]
  ...
  │         ├─ 返回空   → trace "tushare(empty)"，尝试 AKShare
  │         └─ 抛 ProviderError/ProviderUnavailableError → trace "tushare(error: ...)"，尝试 AKShare
  │
  └─ AKShareProvider.is_available()?
       ...
       └─ True  → RateLimiter.acquire() → _http_client.get_kline_daily(...)
            ...
            ├─ 空   → trace "akshare(empty)"
            └─ 异常 → trace "akshare(error: ...)"
  │
  └─ 两源均未返回非空 → DataResult.error(provider="error", source_trace=[...], freshness="empty")
```

> **空结果是否 fallback**：与 Phase 1B-A 的 Router 契约一致——外部 provider 返回空属于「该 provider 无此数据」，Router 继续尝试链中下一个 provider；链耗尽后返回 `DataResult.error`。这与 TA-CN「覆盖该域但空」不 fallback 的语义**不同**（TA-CN 由 capability 映射表判定覆盖性，外部 provider 由链顺序自然推进）。
...
| 两源全返回空 | trace 含 2 个 empty | `provider="error"`, freshness="empty" |
...
| `trade_date` | `trade_date` | 透传（`"YYYYMMDD"` 字符串） | Tushare 原生格式；DailyBar 接受 `YYYY-MM-DD` 或 `YYYYMMDD` |
...
> **单位警示**：Tushare `vol`（手）与 `amount`（千元）与 TA-CN `stock_daily_quotes` 的 `volume`/`amount` 单位可能不同。Phase 1A 的 `DailyBar.from_ta_cn_doc` 直接透传 TA-CN 值。**本阶段不统一跨源单位**——每个 provider 的 `_to_canonical` 按该 provider 的原生单位填入 DailyBar，消费方需知悉跨源单位差异。SPEC §4.3 会固化此决策；统一单位属后续阶段（开放问题 OQ-2）。
...
| `trade_date` | `日期` | `"YYYY-MM-DD"` → 透传（或转 `YYYYMMDD` 统一，SPEC 定） | `YYYY-MM-DD` |
...
> **跨源单位不一致是已知风险**：Tushare vol=手、amount=千元；AKShare volume=股、amount=元。本阶段不归一化，但在 `warnings` 中可选标注来源单位（SPEC 决策）。消费方若做跨源对比需自行换算。
...
- 整个 raw_df 为空 → `fetch()` 返回空 list，Router 视为 empty 继续 fallback。
...
| 重试 | 指数退避（1B-A 框架，默认 3 次，基数 1.0s） | 同上 |
| 超时 | 单请求超时由 HTTP 客户端配置（SPEC 固化默认值，如 30s） | 同上 |
...
|---|---|---|---|---|---|
| Tushare 成功 | `"tushare"` | `FreshnessPolicy.label(...)` | `["tushare(ok)"]` | `[]` 或单位提示 | `None`（Phase 2 才填充） |
| AKShare 兜底成功 | `"akshare"` | `label(...)` | `["tushare(skipped/error...)", "akshare(ok)"]` | `["tushare ..., fell back to akshare"]` | `None` |
...
| 跨源单位不一致（Tushare 手/千元 vs AKShare 股/元）被消费方误用 | 高 | 高 | SPEC §5.2 显式标注单位；`warnings` 可选标注来源单位；文档强调消费方换算责任 | 不归一化（OQ-2） |
...
| 激活后 stub docstring 残留「Phase 1B-B」措辞引发混淆 | 低 | 低 | 本 RFC §2.3 锁定命名为 1D；OQ-1 记录 docstring 同步 | 文档维护 pass |
...
- [ ] Tushare/AKShare 的 `kline_daily` 真实调用经可注入 HTTP 客户端实现。
- [ ] 字段映射表（§5.2）在 SPEC 固化为可执行契约。
- [ ] fake HTTP 客户端单测覆盖主成功、主失败备成功、双失败、空结果、字段缺失、单位标注。
- [ ] Router fallback 测试：kline_daily 链 tushare→akshare 全路径矩阵。
- [ ] 数据合理性断言：fake 返回的 DailyBar 字段值与 fake 原始列一致。
- [ ] `DataResult` provider/source_trace/warnings/quality_score 在各路径下正确（quality_score 恒 None）。
...
1. **stub docstring 措辞同步**：1B-A 的 `tushare.py` / `akshare.py` / `base_external.py` docstring 把真实激活标注为「Phase 1B-B」，与 RFC-03-009 的 1B-B（持久化）冲突。本 RFC 锁定 1D 命名，但是否回头修订已交付 docstring 留作文档维护 pass（避免触碰已 Review 代码）。**建议**：在 Phase 1D 实现卡中顺手更新 docstring 为「Phase 1D」。
...
4. **HTTP 客户端抽象的具体形态**（抽象基类 vs Protocol vs callable 工厂）属 Design 决策，SPEC 只定接口契约。
...
- 现有代码：
  - `skills/data/unified_data/providers/tushare.py`（stub，13 capability）
  - `skills/data/unified_data/providers/akshare.py`（stub，7 capability）
...
| 版本 | 日期 | 更新内容 | 负责人 |
|---|---|---|---|
| V0.1 | 2026-07-18 | 初始创建。Phase 1D 定义：CN `kline_daily` 真实外部 Provider 激活，Tushare 主源 → AKShare 兜底。 | YQuant-Principal |
