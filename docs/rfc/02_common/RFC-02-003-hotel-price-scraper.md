...
重构酒店价格抓取系统,聚焦 Booking 和 Jalan 两个平台,抓取 6 家京都目标酒店未来 30 天的**大床房(ダブル)和双床房(ツイン)**价格(最低价优先,JPY),合并输出 Excel 并通过邮件发送。Trip.com 平台暂不在此版本范围内。每周一 06:10 CST 自动执行。
...
### 3.1 必须目标(Must-Have)
- [ ] 支持 6 家新酒店(完全替换旧 3 家)。
- [ ] 同时抓取**大床房(ダブル)和双床房(ツイン)**两种房型价格,每种取最低价。
- [ ] 平台:Booking + Jalan(Trip.com 不在此版本)。
- [ ] 平台适配器架构,抽象 `BaseHotelScraper` 基类。
- [ ] 标准化数据模型,统一为 `HotelPriceRecord`,新增 `room_category` 字段区分双床/大床。
- [ ] 合并 Excel 输出,两平台结果写入同一工作簿。
- [ ] 使用 `skills/.env` 读取邮件配置,收件人含 `suxn@hyviewgroup.com`。
- [ ] 每周一 06:10 CST 调度执行。
- [ ] 单平台/单酒店失败不影响其他继续抓取和出报表。
...
- 触发条件:
  ...
  - 也支持手工执行单平台或全量抓取。
- 核心处理逻辑:
  1. 读取 `config.yaml` 配置,加载 6 家酒店列表和查询参数。
  2. 初始化 Booking 和 Jalan 平台适配器,并发执行抓取任务。
  3. 每个平台对每个酒店每个日期抓取**大床房最低价**和**双床房最低价**。
  4. 每个平台返回标准化 `HotelPriceRecord` 列表(含 `room_category` 字段)和错误记录。
  5. 聚合结果后导出 `hotel_price_report_YYYY-MM-DD.xlsx`。
  6. 将报表发送至 `EMAIL_RECEIVERS`,邮件复用 `skills/common/utils/email/send_email.py`。
- 正常分支:
...
| platform | string | 平台来源(jalan/booking) | 非空,枚举 |
| checkin_date | date | 入住日期 | 非空 |
| room_category | string | 房型大类(double/twin) | 非空,枚举 |
| room_type | string | 具体房型名称 | 可空,解析失败时为空 |
| price | decimal | 最低价格 | 可空,满房或失败时为空 |
| currency | string | 币种(JPY) | 非空 |
...
### 9.1 功能验收
- [ ] Booking 和 Jalan 两平台均可独立抓取成功(456 仅 Booking)。
- [ ] Excel 输出包含双床房和大床房分别的最低价。
...
| 版本 | 日期 | 更新内容 | 负责人 |
|---|---|---|---|
| V2.0 | 2026-06-14 | 重大重构：6 家新酒店替换旧 3 家；房型变更为大床房+双床房；去掉 Trip.com；更新 RFC | YQuant |
