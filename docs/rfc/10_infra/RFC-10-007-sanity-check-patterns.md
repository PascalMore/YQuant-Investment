...
本 RFC 建议新增一个统一的工程质量 skill：`skills/infra/sanity-check/`。该 skill 不替代业务 validation，也不替代运行时 error handling；它沉淀“数据进入业务逻辑之前，对输入和环境的最后一道合理性闸门”，为 worker 在新增功能、修改 CLI、接入文件/数据库/日期字段时提供可复制的 Python 标准模板。
...
- [ ] 新增统一 skill：`skills/infra/sanity-check/`。
...
本任务是 Quick Flow T1（RFC/SPEC/Design 合并 task）。T1 只产出三层文档，不直接实现 skill 文件；后续 T2 Implement 按 DESIGN-10-007 的文件清单创建 `skills/infra/sanity-check/`。这是为了保留 Principal / Developer 角色边界，避免 T1 同时设计和实现。
