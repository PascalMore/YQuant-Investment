...
| Portfolio 增量同步 | IMPLEMENTED | `StockPoolTransitionPipeline.run_incremental_transition()` 调用状态机迁移 | `daily_processor.py`, `StockPoolTransitionPipeline`, `ZoneRuleEngine` |
...
当前四区分类规则不在本文硬编码。统一阈值来源为 `skills/research/argus/config/zone_rules_template.yaml`，由 `ZoneRuleEngine` 加载执行：

| Zone | 当前规则 |
|:--|:--|
| `CONVICTION` | 满足 `argus_signal_pool.entry_rules.conviction`，当前 YAML 要求 `bayesian_score`、产品数、共识与拥挤度同时达标 |
| `CANDIDATE` | 满足 `argus_signal_pool.entry_rules.candidate`，当前 YAML 要求 contributing products `>= 2`，并受 `crowding_max: DANGER` 约束 |
| `WATCH` | 满足 `argus_signal_pool.entry_rules.watch` |
| `SCAN` | `argus_signal_pool.entry_rules.scan` residual rule |

旧代码中的 `ZONE_THRESHOLDS`、`zone_thresholds` 或 confidence-only 判断仅作为兼容历史，不是当前文档依据。Darwin override 也是 YAML 规则的一部分：命中 guard 时只做 floor-only `CANDIDATE`，不会直接强制 `CONVICTION`。
...
daily_processor 读取 previous signal_pool
    ↓
StockPoolTransitionPipeline.run_incremental_transition()
    ↓
...
同步逻辑会比较当前与前序 signal_pool，由状态机生成 entry / promote / demote / exit / update 等动作摘要。Portfolio stock_pool 的 zone 由 `classify_transition()` 基于当前 zone 与 YAML retention / promotion / exit 规则驱动，不由 ingestion 直接覆盖。为了处理节假日缺口，`daily_processor` 会按股票查找该股票最近一次 prior signal_pool 记录，而不只依赖前一交易日。
...
|:--|:--:|:--|:--|:--|
| **SCAN** | 无限 | YAML residual rule：未满足更高 zone 时默认进入 | YAML `exit_rules`：`missing_from_current_signal_pool` | (不映射) |
| **WATCH** | 原始设计基线，当前实现不由本文定义 | YAML `argus_signal_pool.entry_rules.watch` | YAML `watch_retention` 失败或 `exit_rules` 命中 | OBSERVE 候选 |
| **CANDIDATE** | 原始设计基线，当前实现不由本文定义 | YAML `argus_signal_pool.entry_rules.candidate`，当前要求 products `>= 2` | YAML `candidate_retention` 失败或 `exit_rules` 命中 | OBSERVE / CANDIDATE |
| **CONVICTION** | 原始设计基线，当前实现不由本文定义 | YAML `argus_signal_pool.entry_rules.conviction` | YAML `conviction_retention` 失败或 `exit_rules` 命中 | CANDIDATE 晋升 |

### 1.2 动态容量机制 (REV-016) {#ARGUS-05:dynamic-capacity}

当市场处于不同状态时，池容量自动调整。当前运行实现未启用动态容量阈值；若 rearm，应作为 `zone_rules_template.yaml` 或独立 capacity 配置声明，不在 Web / ingestion 调用方硬编码。
...
- `bayesian_score`、产品数、共识和拥挤度满足 `zone_rules_template.yaml` 对应 entry rule
- 数据完整性检查: 标的有近 5 日行情数据

**阶段二 -- 净化过滤 (Purification Filter)**:
- 拥挤度检查: 目标 zone 受 YAML `crowding.zone_limits` 约束
- 矛盾信号检查: 有高信誉产品卖出时降低信号权重
- 流动性检查: 若纳入实现，必须作为 YAML 或独立配置项声明，不在调用方硬编码

### 1.4 升降级路径 / Promotion & Demotion Path {#ARGUS-05:promotion-demotion}

```
SCAN ──[zone_rules_template.yaml: promote_rules.scan_to_watch]──> WATCH
WATCH ──[zone_rules_template.yaml: promote_rules.watch_to_candidate]──> CANDIDATE
CANDIDATE ──[zone_rules_template.yaml: promote_rules.candidate_to_conviction]──> CONVICTION

CONVICTION ──[zone_rules_template.yaml: demote_rules.conviction_retention 失败]──> CANDIDATE
CANDIDATE ──[zone_rules_template.yaml: demote_rules.candidate_retention 失败]──> WATCH
WATCH ──[zone_rules_template.yaml: demote_rules.watch_retention 失败]──> SCAN
任何区域 ──[zone_rules_template.yaml: exit_rules missing_from_current_signal_pool]──> inactive
...
| IC 模式 | 触发 | ARGUS 提供 |
|:--|:--|:--|
| IC-FAST (快审) | CONVICTION 区高优先级标的，具体触发阈值由配置或回测校准决定 | 信号摘要 + 产品列表 + 共识强度 |
...
| 版本 | 日期 | 变更内容 |
|:--|:--|:--|
| 2.0.1 | 2026-06-07 | 移除股票池硬编码阈值描述，统一指向 `zone_rules_template.yaml` 与 `ZoneRuleEngine`；更新 Portfolio 增量同步为 `StockPoolTransitionPipeline.run_incremental_transition()` 状态机路径 |
...
*ARGUS-05 v2.0.1 -- Stock Pool Management & Web Interface*
