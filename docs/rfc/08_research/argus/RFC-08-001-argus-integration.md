...
| `argus_darwin_event` | 达尔文时刻（拥挤度峰值） |
| `argus_consensus_direction` | 共识方向详情 |

#### 4.2.3 Argus Decision Layer（决策层）

| Decision 表名 | 说明 |
|---|---|
| `argus_stock_pool` | 四区动态股票池（SCAN/WATCH/CANDIDATE/CONVICTION） |
| `argus_stock_pool_history` | 股票池历史快照 |
