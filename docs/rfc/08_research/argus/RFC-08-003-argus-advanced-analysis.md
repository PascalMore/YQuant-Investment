...
#### 3.4.2 Hysteresis 机制
Portfolio 侧迁移采用 independent retention thresholds：晋级阈值高于留存阈值，避免股票在相邻 zone 间因边际分数波动反复跳转。
...
`ZoneRuleEngine.classify_transition()` 的执行顺序固定为：

1. `exit`：当前股票从当日 Argus signal pool 消失时，标记 lifecycle inactive，不把它降级为 SCAN。
2. `promote`：若下一 zone 晋级规则通过，最多上移一档。
3. `demote`：若当前 zone 留存规则失败，最多下移一档。
4. `retain`：既不晋级也不降级时保留原 zone，并记录审计原因。

这个顺序保证“信号消失”与“信号变弱”不会混淆，也让 Portfolio stock pool 的增量审计可复现。

#### 3.4.4 Darwin floor 与 crowding zone limits
