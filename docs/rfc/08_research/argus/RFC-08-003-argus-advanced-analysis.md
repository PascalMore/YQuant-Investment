...
| 版本号 | 日期 | 更新内容 | 负责人 |
|---|---|---|---|
| V0.3 | 2026-06-06 | 明确 Signal ID 幂等性和 Hysteresis 状态机为当前实现契约 | YQuant |
| V0.2 | 2026-06-06 | Phase 3：补充 bayesian_score 参与初始 zone 分类、hysteresis、统一 promote/demote/exit 规则与 crowding zone limits | YQuant |
...
| 规则单一来源 | zone 初始分类和组合池迁移统一使用 ZoneRuleEngine 与 YAML 配置，避免阈值散落在调用方 |
...
### 3.4 Phase 3：统一 ZoneRuleEngine 规则
#### 3.4.1 bayesian_score 参与初始分类
Phase 3 将 `bayesian_score` 设为 Argus 初始 zone 分类的主评分，不再直接使用原始 `confidence` 阈值决定 `pool_zone`。`confidence` 仅作为兼容字段和缺失 `bayesian_score` 时的 fallback。

初始分类输入为单股单日 `stock_pool_record`：
...
crowding_level
darwin_moment / darwin_confidence
```

规则配置位于 `skills/research/argus/config/zone_rules_template.yaml`：

| 目标 zone | 主条件 |
|---|---|
| CONVICTION | 高 `bayesian_score`、至少 3 个产品、较高共识，且拥挤度不超过配置上限 |
| CANDIDATE | 中高 `bayesian_score`、至少 2 个产品、有有效共识，且未触发过高拥挤限制 |
| WATCH | 低门槛正向评分和基础共识 |
| SCAN | residual entry rule，未满足更高 zone 时默认进入 |

#### 3.4.2 Zone 转换 - Hysteresis 机制
Portfolio 侧迁移采用 independent retention thresholds：晋级阈值高于留存阈值。设计目的是避免边界震荡，即 `bayesian_score` 在阈值附近波动导致 zone 频繁跳动。

| 当前 zone | 晋级规则 | 留存失败后的降级 |
|---|---|---|
| SCAN | `scan_to_watch` | 不降级 |
| WATCH | `watch_to_candidate` | `watch_retention` 失败则回到 SCAN |
| CANDIDATE | `candidate_to_conviction` | `candidate_retention` 失败则回到 WATCH |
| CONVICTION | 顶层终态 | `conviction_retention` 失败则回到 CANDIDATE |

每次运行最多移动一步，避免一次日更将标的从 SCAN 直接推至 CONVICTION 或从 CONVICTION 直接打回 SCAN。

#### 3.4.3 promote / demote / exit 统一语义
`ZoneRuleEngine.classify_transition(metrics, current_zone)` 的执行顺序固定为：

1. `exit` 优先：`missing_from_signal_pool` 时直接 `EXIT`，标记 lifecycle inactive，不把它降级为 SCAN。
2. `promote` 限一步：若下一 zone 晋级规则通过，最多上移一档。
3. `demote` 限一步：若当前 zone 留存规则失败，最多下移一档。
4. `retain`：metrics 无显著变化、既不晋级也不降级时保留原 zone，并记录审计原因。

这个顺序保证“信号消失”与“信号变弱”不会混淆，也让 Portfolio stock pool 的增量审计可复现。旧方案 `classify_initial_zone()` 每日重新计算，无历史状态，不能表达 retention buffer。

#### 3.4.4 Signal ID 生成策略（幂等性设计）
Argus product-level signal 使用确定性 `signal_id`，生成策略如下：

- **设计原则**：`signal_id` 基于业务键 `date, product_code, wind_code, signal_type` 的确定性 hash。
- **格式**：`argus:{sha256_hash}` 前 20 字符。
- **目的**：支持 upsert 语义，日度任务重跑不会重复生成记录，保证 `08_research_argus_signal` 可审计、可重放。
- **对比**：旧方案使用随机 UUID，每次重跑产生新记录，导致下游只能事后按最新时间戳去重。

#### 3.4.5 Darwin floor 与 crowding zone limits
Darwin override 采用 floor-only 策略：当 `darwin_moment=true` 且满足 `bayesian_score` 或 `darwin_confidence` guard 时，最低提升到 `CANDIDATE`；不直接强制 `CONVICTION`。

拥挤度是 zone 的上限约束而非加分项：

| crowding_level | 规则影响 |
|---|---|
| LOW / MEDIUM | 不限制 WATCH/CANDIDATE，满足其他条件可进入高 zone |
| HIGH | 允许进入 CONVICTION 的上限由 YAML 显式配置 |
| DANGER | 阻断需要更低拥挤度的高 zone，只保留到允许的最高 zone |
| UNKNOWN | 按配置缺省 rank 处理，调用方应保守解释 |
...
```json
{
  ...
  "phase_links": {
    "source_signal_ids": ["argus:5f2c9b7e1a4d8c0b3e91"],
