...
| 关联 Design | DESIGN-03-007 / DESIGN-03-008 / DESIGN-03-009（待 T2 Design 阶段产出 Phase 1D 章节） |
...
- **Phase 1D** = External Provider Activation。**不得**称「Phase 1B-B」（1B-B 已由 RFC-03-009 锁定为持久化缓存平面）。1B-A provider stub docstring 残留的「Phase 1B-B」字样属历史前瞻标注，不构成事实契约（RFC-03-012 §2.3）。
...
- **单位差异（已知风险，本阶段不归一化）**：Tushare `vol`=手、`amount`=千元；AKShare `成交量`=股、`成交额`=元；TA-CN `stock_daily_quotes` 单位由 TA-CN 决定。Phase 1D 的 `_to_canonical` **透传 provider 原生单位**，消费方自负跨源换算责任（RFC-03-012 §5.2 / OQ-2）。
...
3. **HTTP 客户端抽象**：新增轻量客户端接口（构造注入或工厂），封装真实 SDK 调用；提供 `FakeKlineClient` 用于单测。真实 Tushare/AKShare 客户端实现属 Design 决策。
...
- [ ] TushareProvider：`kline_daily` 的 `fetch()` 改为真实调用路径（经可注入 HTTP 客户端）；`_to_canonical()` 实现 Tushare 列 → `list[DailyBar]`；其余 12 capability 保持 stub。
- [ ] AKShareProvider：`kline_daily` 的 `fetch()` 同上；`_to_canonical()` 实现 AKShare 列 → `list[DailyBar]`；其余 6 capability 保持 stub。
- [ ] HTTP 客户端抽象：接口契约（§4.4）+ `FakeKlineClient`（测试用）。
- [ ] 字段映射契约：Tushare / AKShare 原始列 → `DailyBar` 精确映射表（§4.3），含日期格式、单位、空值、缺失列处理。
- [ ] 确定性行为：空结果、字段缺失、超时、限流、重试、可用性判定、双源失败的终态 `DataResult` 全部可执行、可测试。
- [ ] `DataResult` 的 `provider` / `source_trace` / `warnings` / `quality_score` 在真实调用路径下与 Phase 1B-A / Phase 2 契约一致（`quality_score` 恒 `None`）。
- [ ] 全量文件清单（新增 / 修改 / 测试）精确到文件路径（§8）。
- [ ] fake HTTP 客户端测试矩阵覆盖全部行为契约（§9）。

### 2.2 Out of Scope（Phase 1D 不做）

- [ ] 不激活 Tushare 其余 12 条 capability（kline_weekly / adj_factor / financial 三表 / daily_basic / calendar / metadata / news）。
- [ ] 不激活 AKShare 其余 6 条 capability。
- [ ] 不实现 Sector Router（DESIGN-03-010 已明确不在范围）。
- [ ] 不做生产副作用：无 MongoDB DDL / 真实 Mongo 写入 / materialization 写入 / cache 写入 / AuditLogger 真实写入 / QualitySummary 启用 / cron/systemd/webhook。
- [ ] 不把真实网络 smoke 作为本卡或实现卡的执行前提（§9.3）。
- [ ] 不归一化跨源单位（Tushare 手/千元 vs AKShare 股/元）——消费方自负换算（OQ-2）。
- [ ] 不改造 `datetime.utcnow()` 技术债（§7.4 / OQ-3）。
- [ ] 不修改 `SecurityId` / `DataResult` / `Market` / `Capability` / `DailyBar` / `FreshnessPolicy` 公共契约。
- [ ] 不修改 `DataRouter` / `ProviderRegistry` / `UnifiedDataClient` / `TA_CNMongoAdapter` 代码。
- [ ] 不修改 TA-CN 子项目代码（`skills/apps/TradingAgents-CN/**`）与 TA-CN 无前缀集合。
- [ ] 不修改 Phase 1A 的 14 个域入口方法行为（继续直连 TA-CN adapter）。
- [ ] 不修改 RFC/SPEC/Design 文档模板（编排层不改模板，P-7）。
- [ ] 不修改 RFC-03-012（本卡只产出 SPEC）。
...
| 编号 | 行为 | 输入 | 输出 | 错误/边界 |
|---|---|---|---|---|
| EP-101 | TushareProvider.fetch(kline_daily) 真实路径 | `domain="market_data", operation="kline_daily", security_id, **params`（params 含 `start_date`/`end_date`/`limit`，均可选） | `list[DailyBar]`（经 `_to_canonical` 转换） | is_available()=False → `ProviderUnavailableError`；HTTP 异常 → `ProviderUnavailableError`/`ProviderError` |
...
| EP-104 | 空结果处理 | HTTP 客户端返回空 DataFrame / 空 list | `fetch()` 返回空 `list`（`[]`） | Router 视为 empty，继续 fallback |
| EP-105 | 字段缺失处理 | raw_df 缺关键字段（`close` 或 `trade_date`） | 该行丢弃，不计入 list | 缺失列（整个 raw_df 无 `vol` 列）→ 该字段 DailyBar=None 或抛映射错误（§4.3 决策） |
| EP-106 | RateLimiter 激活 | 每次 fetch 前 | `RateLimiter.acquire()`（沿用 1B-A 框架） | 真实网络下验证；超限等待 |
| EP-107 | 重试激活 | HTTP 异常（超时/5xx） | 指数退避重试（沿用 1B-A 框架，默认 max_attempts、backoff_base 由构造参数） | 配额耗尽（429 类）不重试，直接 `ProviderUnavailableError` |
...
| HC-104 | 真实 Tushare 客户端 | token（经环境变量） | 调 `tushare.pro_bar`/`daily` | Design 决策具体 SDK 调用 |
| HC-105 | 真实 AKShare 客户端 | 无 token | 调 `akshare.stock_zh_a_hist` | Design 决策具体 SDK 调用 |

> HC-104 / HC-105 的具体 SDK 调用方式（`tushare.pro_api(token).daily(...)` vs `tushare.pro_bar(...)`；`akshare.stock_zh_a_hist` 参数 `adjust`/`period`）属 **Design 决策**，SPEC 只定接口契约。
...
def fetch(
    ...
    **params: Any,
) -> list["DailyBar"]:  # kline_daily 分支返回 list[DailyBar]；其余分支返回 pd.DataFrame（stub）
```

> **返回类型差异**：kline_daily 分支返回 `list[DailyBar]`（canonical），其余 capability 分支返回 `pd.DataFrame`（stub，沿用 1B-A）。Router 的 `provider.fetch(...)` 调用点（`router.py:906`）对返回类型透明——它把返回值交给 DataResult 包装逻辑。Design 阶段需确认 Router 如何把 `list[DailyBar]` 装入 `DataResult.data`（预期：`DataResult.data = list[DailyBar]`，与 TA-CN 路径的 `get_daily_bars` 返回类型一致）。
...
| `trade_date` | `trade_date` | 透传（`"YYYYMMDD"`） | YYYYMMDD | 必填；缺失→该行丢弃 |
| `open` | `open` | `float()` | 元 | None |
| `high` | `high` | `float()` | 元 | None |
| `low` | `low` | `float()` | 元 | None |
| `close` | `close` | `float()` | 元 | 关键字段；None→该行丢弃 |
| `pre_close` | `pre_close` | `float()` | 元 | None |
| `change` | `change` | `float()` | 元 | None |
| `pct_chg` | `pct_chg` | `float()` | 百分比（已×100） | None |
| `volume` | `vol` | `float()` | **手**（1手=100股） | None |
| `amount` | `amount` | `float()` | **千元** | None |
...
|---|---|---|---|---|
| `symbol` | 入参 `symbol` | 透传 | 6 位代码 | 必填 |
| `trade_date` | `日期` | `"YYYY-MM-DD"` 透传（或转 YYYYMMDD，Design 定） | YYYY-MM-DD | 必填；缺失→该行丢弃 |
| `open` | `开盘` | `float()` | 元 | None |
| `high` | `最高` | `float()` | 元 | None |
| `low` | `最低` | `float()` | 元 | None |
| `close` | `收盘` | `float()` | 元 | 关键字段；None→该行丢弃 |
| `pre_close` | — | `close - 涨跌额` 或 `None` | 元 | None（AKShare 不直接提供） |
| `change` | `涨跌额` | `float()` | 元 | None |
| `pct_chg` | `涨跌幅` | `float()` | 百分比 | None |
| `volume` | `成交量` | `float()` | **股** | None |
| `amount` | `成交额` | `float()` | **元** | None |
| `turnover_rate` | `换手率` | `float()` | 百分比 | None |
| `volume_ratio` | — | `None` | — | 恒 None |
...
- **整 raw_df 空**：`fetch()` 返回 `[]`，Router 视为 empty 继续 fallback。

> **单位警示（必现在 warnings）**：当 `kline_daily` 由 Tushare 成功返回时，DataResult.warnings 可选追加 `"volume unit: 手 (tushare)"`；AKShare 成功时追加 `"volume unit: 股 (akshare)"`。此为消费方换算提示，**非错误**。Design 阶段决定是否默认开启（可配置）。
...
class KlineClient(Protocol):
    """可注入的 kline_daily HTTP 客户端抽象。

    实现者：
    - 真实 Tushare 客户端（调 tushare SDK）
    - 真实 AKShare 客户端（调 akshare SDK）
    - FakeKlineClient（测试用，按 fixture 返回）
    ...
    def get_kline_daily(
        self,
        security_id: "SecurityId",
        start_date: str | None = None,   # YYYYMMDD（Tushare 风格）或 YYYY-MM-DD，Design 统一
...
class TushareProvider(BaseExternalProvider):
    def __init__(
        ...
        *,
        ...
        http_client: KlineClient | None = None,  # [1D 新增] None → 默认真实客户端
        request_timeout_seconds: float = 30.0,   # [1D 新增] 单请求超时
    ) -> None: ...
...
class AKShareProvider(BaseExternalProvider):
    def __init__(
        ...
        *,
        ...
        http_client: KlineClient | None = None,  # [1D 新增]
        request_timeout_seconds: float = 30.0,
    ) -> None: ...
```

> `request_timeout_seconds` / `rate_limit_rpm` / `retry_*` 的具体默认值（30.0 / 200 / 3 / 1.0）是 SPEC 给出的**初始默认**，Design 阶段可基于真实网络验证调整。这些值**不得**硬编码在调用处，必须经构造参数或 config 注入（P-3：不硬编码阈值）。

### 4.5 错误/降级矩阵（DataResult 终态，kline_daily）

| 场景 | `provider` | `freshness` | `source_trace` | `warnings` | `quality_score` |
|---|---|---|---|---|---|
| Tushare 成功 | `"tushare"` | `FreshnessPolicy.label(...)` | `["tushare(ok)"]` | `[]` 或单位提示 | `None` |
| AKShare 兜底成功 | `"akshare"` | `label(...)` | `["tushare(skipped: unavailable)", "akshare(ok)"]` | `["tushare unavailable, fell back to akshare"]` + 单位提示 | `None` |
| Tushare 异常 + AKShare 成功 | `"akshare"` | `label(...)` | `["tushare(error: ...)", "akshare(ok)"]` | `["tushare error, fell back to akshare"]` | `None` |
...
| 两源全空 | `"error"` | `"empty"` | `["tushare(empty)", "akshare(empty)"]` | `["no data from any provider"]` | `None` |
| `provider="tushare"` forced + 成功 | `"tushare"` | `label(...)` | `["tushare(ok)"]` | 单位提示 | `None` |
...
| 5 | 跨源单位不归一化 | §0 单位差异声明；warnings 单位提示；OQ-2 | §0 / §4.3 / §10 |
...
| 9 | 测试分层：fake HTTP / 可注入客户端单测 / Router fallback / 数据合理性 | §9 测试矩阵 | §9 |
...
> **P-3（不硬编码阈值）**：SPEC 中所有数字（200/3/1.0/30.0/0.5）均为**初始默认值**，实现时必须从 `config.yaml` 对应字段读取或经构造参数注入，**不得**硬编码在调用处。Design 阶段可基于真实网络验证调整默认值。
...
理由：1D 不触 LocalMongoAdapter / CacheManager（1B-B 持久化层，本阶段不写入）。统一 UTC-aware 改造是横切关注点，应作为独立技术债卡（OQ-3），避免扩散 1D 范围。Router/models/freshness 的 `datetime.now(timezone.utc).replace(tzinfo=None)` 是 UTC-aware 后剥 tz，语义等价且非弃用路径，不在本债范围内。
...
| 路径 | 说明 |
|---|---|
| `skills/data/unified_data/providers/kline_client.py` | `KlineClient` Protocol + `FakeKlineClient`（测试用）；真实 Tushare/AKShare 客户端实现是否独立文件由 Design 定 |
| `tests/data/unified_data/test_kline_client.py` | `KlineClient` / `FakeKlineClient` 单元测试 |
| `tests/data/unified_data/test_providers_kline_daily.py` | TushareProvider/AKShareProvider 的 kline_daily 激活路径单测（注入 FakeKlineClient） |
| `tests/data/unified_data/test_router_kline_daily_fallback.py` | Router fallback：kline_daily 链 tushare→akshare 全路径矩阵（注入 fake providers） |

> 真实 Tushare/AKShare 客户端实现（HC-104/105）的文件位置由 Design 决定（可能在 `providers/tushare.py` / `akshare.py` 内，或独立 `providers/_real_clients.py`）。SPEC 不锁定具体文件名，只锁接口契约（§4.4）。

### 8.2 修改文件

| 路径 | 修改内容 |
|---|---|
| `skills/data/unified_data/providers/tushare.py` | `fetch()` 按 capability 分支：`kline_daily` 走真实路径（经 `http_client`）+ `_to_canonical` 实现 Tushare 列→list[DailyBar]；其余 12 capability 保持 stub。构造新增 `http_client`/`request_timeout_seconds` 可选参数。docstring 把「Phase 1B-B」改为「Phase 1D」（OQ-1 顺手处理） |
| `skills/data/unified_data/providers/akshare.py` | 同上（kline_daily 激活；其余 6 capability 保持 stub；docstring 措辞同步） |
| `skills/data/unified_data/providers/base_external.py` | `_to_canonical` 可考虑提升为支持 kline_daily 分支的 hook（或保留 no-op，子类各自 override）；docstring 措辞同步。**具体形态由 Design 定**，SPEC 只要求子类能 override |
| `skills/data/unified_data/__init__.py` | 导出 `KlineClient` / `FakeKlineClient`（如新增） |
| `skills/data/unified_data/providers/_stub_columns.py` | 不改内容；如需为 kline_daily 标注「已激活，不走 stub」可在注释说明（可选） |

### 8.3 不改动文件（明确列出）

- 见 §7.3 禁止事项表。特别注意：`models/**`、`router.py`、`registry.py`、`freshness.py`、`client.py`、`adapters/ta_cn_mongo_adapter.py`、`local_mongo_adapter.py`、`cache_manager.py`、`audit/**`、`quality/**`、`exceptions.py`、`config.py` 均不改。
- RFC-03-012 不改（本卡只产出 SPEC）。
- RFC/SPEC/Design 文档模板不改（P-7）。
...
| 测试编号 | 测试目标 | 覆盖功能 | mock/注入方式 | 断言 |
|---|---|---|---|---|
| UT-KC-001 | FakeKlineClient 返回 fixture DataFrame | HC-103 | 直接构造 | `get_kline_daily(...)` 返回配置的 DataFrame |
| UT-KC-002 | FakeKlineClient 抛 ProviderUnavailableError | HC-103 | 配置异常 | raise 指定异常 |
| UT-KC-003 | FakeKlineClient 返回空 DataFrame | HC-103 | 配置空 | 返回空 DataFrame |
| UT-TP-201 | Tushare kline_daily 真实路径成功 | EP-101 | 注入 FakeKlineClient 返回 Tushare 列 DataFrame | 返回 `list[DailyBar]`，字段映射正确（§4.3.1） |
| UT-TP-202 | Tushare kline_daily 空结果 | EP-104 | FakeKlineClient 返回空 | `fetch()` 返回 `[]` |
| UT-TP-203 | Tushare kline_daily 字段缺失（缺 close 列） | EP-105 | FakeKlineClient 返回缺 close 的 DataFrame | raise `ProviderError("missing required column: close")` |
| UT-TP-204 | Tushare kline_daily 行级 close=None 丢弃 | EP-105 | DataFrame 含 close=NaN 行 | 该行不入 list，其余行入 |
| UT-TP-205 | Tushare kline_daily HTTP 超时 | EP-101 | FakeKlineClient 抛 ProviderUnavailableError | raise（Router 侧 fallback） |
| UT-TP-206 | Tushare 其余 12 capability 保持 stub | EP-103 | 调 `kline_weekly` 等 | 返回 stub DataFrame（空壳），不发网络（FakeKlineClient 未被调用） |
| UT-TP-207 | Tushare 单位标注（vol=手/amount=千元） | §4.3.1 | FakeKlineClient 返回含 vol/amount 的 DataFrame | DailyBar.volume/amount 透传；warnings 含单位提示（若开启） |
| UT-TP-208 | Tushare is_available 不变 | EP-108 | monkeypatch TUSHARE_TOKEN | token 存在+import → True；否则 False（沿用 1B-A） |
| UT-AK-201 | AKShare kline_daily 真实路径成功 | EP-102 | 注入 FakeKlineClient 返回 AKShare 中文列 DataFrame | 返回 `list[DailyBar]`，字段映射正确（§4.3.2） |
| UT-AK-202 | AKShare kline_daily 空结果 | EP-104 | 空 DataFrame | 返回 `[]` |
| UT-AK-203 | AKShare 缺 `收盘` 列 | EP-105 | 缺 close 列 | raise `ProviderError("missing required column: 收盘")` |
| UT-AK-204 | AKShare 行级 close=None 丢弃 | EP-105 | close=NaN 行 | 该行丢弃 |
| UT-AK-205 | AKShare 单位标注（volume=股/amount=元） | §4.3.2 | 含 成交量/成交额 | DailyBar 透传；warnings 含单位提示 |
| UT-AK-206 | AKShare 其余 6 capability 保持 stub | EP-103 | 调 `kline_weekly` 等 | stub DataFrame，不发网络 |
| UT-AK-207 | AKShare is_available 不变 | EP-108 | mock import | import 成功 → True（沿用 1B-A） |
| UT-DR-301 | Router kline_daily TA-CN 命中（不调外部） | SPEC-03-008 DR-101 | FakeDatabase 有数据 + FakeProvider(external) | `provider="ta_cn_internal"`，external call_log 空 |
| UT-DR-302 | Router kline_daily TA-CN 未覆盖场景 → tushare 成功 | §4.5 | FakeProvider(tushare ok) | `provider="tushare"` |
| UT-DR-303 | Router kline_daily tushare 失败 → akshare 兜底 | §4.5 | tushare raise + akshare ok | `provider="akshare"`, warnings 含 fallback |
| UT-DR-304 | Router kline_daily 两源全失败 | §4.5 | 两源 raise | `provider="error"`, source_trace 2 个 error |
| UT-DR-305 | Router kline_daily 两源全空 | §4.5 | 两源返回空 | `provider="error"`, freshness="empty" |
| UT-DR-306 | Router kline_daily 两源全不可用 | §4.5 | 两源 is_available=False | `provider="error"`, source_trace 2 个 skipped |
| UT-DR-307 | Router kline_daily provider="tushare" forced | §4.5 | provider="tushare" | 只走 tushare，不 fallback |
| UT-DR-308 | Router kline_daily force_refresh 跳过 TA-CN | SPEC-03-008 DR-102 | TA-CN 有数据 + force_refresh=True | `provider="tushare"`, TA-CN 未调 |
| UT-DR-309 | Router kline_daily quality_score 恒 None | §4.5 | 任意成功路径 | `DataResult.quality_score is None` |
| UT-SEC-401 | is_available 不泄露 token 值 | §7.2 | monkeypatch TUSHARE_TOKEN="secret" | is_available 返回 True/False，不返回/不打印 "secret" |
| UT-SEC-402 | 错误信息不含 token | §7.2 | FakeKlineClient 抛含 token 的异常（模拟） | provider 捕获后 re-raise 的 message 不含 token |
| UT-SEC-403 | FakeKlineClient 不读环境变量 | §7.2 | 构造 FakeKlineClient | 不调 os.environ |
...
- **非空性**：成功路径的 `list[DailyBar]` 必须非空（除非 fixture 显式空）。
- **不返回 stub**：kline_daily 激活路径**不得**返回 `_stub_columns.stub_dataframe_for` 的空壳 DataFrame（断言 `not isinstance(result, pd.DataFrame) or result 不等于 stub`）。
- **关键字段不为 None**：成功路径的 DailyBar `close`/`trade_date` 必须非 None（除非该行被丢弃）。

### 9.4 回归测试

- Phase 1B-A 的 `test_providers.py`（stub 路径）全部通过——验证 kline_daily 激活不破坏其余 capability 的 stub 行为。
- Phase 1B-A 的 `test_router_internal_first.py` 全部通过——验证 Router 编排不变。
...
- **真实网络下 RateLimiter/重试行为**：受控 smoke 阶段验证。
...
| 编号 | 验收项 | 验证方式 |
|---|---|---|
| A-001 | TushareProvider kline_daily 经可注入 HTTP 客户端真实调用 | UT-TP-201~205 |
| A-002 | AKShareProvider kline_daily 同上 | UT-AK-201~205 |
...
| A-006 | AKShare 其余 6 capability 保持 stub | UT-AK-206 |
...
| A-010 | 单位标注（vol/volume, amount 单位差异）在 warnings 体现（若开启） | UT-TP-207, UT-AK-205 |
| A-011 | is_available 不泄露 token | UT-SEC-401 |
| A-012 | 错误信息不含 token | UT-SEC-402 |
| A-013 | 不返回 stub/假数据冒充真实 | UT 数据合理性断言（§9.3） |
...
- `fetch()` 签名不变；kline_daily 分支返回类型从 stub DataFrame 改为 `list[DailyBar]`——这是**预期行为变更**（激活），1B-A 测试中对 kline_daily 返回 stub DataFrame 的断言需更新为 list[DailyBar]（Implement 阶段同步）。
...
| 跨源单位不一致被消费方误用 | 高 | 高 | §0 声明 + warnings 单位提示 + OQ-2 |
| 真实网络 smoke flaky | 中 | 中 | smoke 非执行前提（§9.5）；fake 客户端单测为主 |
| token 泄露 | 低 | 高 | P-10 + UT-SEC-401/402 |
| 限流/重试真实行为偏差 | 中 | 中 | 框架沿用 1B-A，真实网络下验证 |
| kline_daily 返回类型变更破坏 1B-A 测试 | 高 | 中 | §11.1 Implement 同步更新 1B-A 测试断言 |
| datetime.utcnow() 扩散 | 低 | 低 | §7.4 不触；OQ-3 |

### 移交 Design 阶段的待决项

1. HTTP 客户端抽象的具体形态（Protocol vs ABC vs callable 工厂）—— SPEC 已定接口契约（§4.4），形态属 Design。
2. 真实 Tushare 客户端具体 SDK 调用（`pro_api(token).daily(...)` vs `pro_bar(...)`）—— Design 决策。
3. 真实 AKShare 客户端 `stock_zh_a_hist` 的 `adjust`/`period` 参数默认值 —— Design 决策。
4. AKShare `trade_date` 格式（YYYY-MM-DD 透传 vs 转 YYYYMMDD 统一）—— Design 决策。
5. 单位提示 warnings 是否默认开启 —— Design 决策（可配置）。
6. `_to_canonical` 是 base 类 hook 还是子类各自 override —— Design 决策。
7. kline_daily 返回 `list[DailyBar]` vs `pd.DataFrame` 的 Router 包装逻辑确认 —— Design 需确认 Router 如何把 list[DailyBar] 装入 DataResult.data。
...
- DESIGN-03-007 / DESIGN-03-008 / DESIGN-03-009（待 T2 Design 补 Phase 1D 章节）
- 现有代码：
  - `skills/data/unified_data/providers/tushare.py`（stub，待激活 kline_daily）
  - `skills/data/unified_data/providers/akshare.py`（stub）
  ...
  - `skills/data/unified_data/router.py`（internal-first，`provider.fetch` L906）
  - `skills/data/unified_data/models/domain/market_data.py`（`DailyBar`）
