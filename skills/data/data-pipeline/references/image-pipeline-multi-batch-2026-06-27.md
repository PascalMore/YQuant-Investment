...
**2026-06-27 晚间二次修复（贪心匹配 bug，commit `b277281`）**：为 28 张"未跑" jpg 创建精确时间戳 xlsx marker 后，脚本**仍报 28 张未跑**。根因：Pass 1 缺失 — 贪心窗口匹配按 jpg 时间戳排序消费产物，时间早的 jpg 抢了后面 jpg 的精确匹配 xlsx。修复：加 Pass 1（`jpg_ts == product_ts` 精确匹配）在 Pass 2（贪心窗口）之前。修复后 36/36 已跑。

**xlsx marker 技术**：用户确认图片已处理但脚本报"未跑"时，创建最小 xlsx（`portfolio_{YYYYMMDD}_{HHMMSS}.xlsx`，内容仅 `['marker']` 一行）作为"已跑"标记。
