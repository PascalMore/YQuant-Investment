...
- Intake 和 Closeout：`YQuant`，默认模型 `zai/glm-5.1`，第一 fallback `minimax/MiniMax-M2.7`。
- RFC/SPEC/Design：`YQuant-Codex-Principal`，默认模型 `codex/gpt-5.5`，fallback `zai/glm-5.1`。
- Implement：`YQuant-Developer-Engineer`，默认模型 `minimax/MiniMax-M2.7`，fallback `zai/glm-5.1`。
- Verify：`YQuant-Test-Engineer`，默认模型 `minimax/MiniMax-M2.7`，fallback `zai/glm-5.1`。
- Review：`YQuant-Reviewer-Principal`，默认模型 `zai/glm-5.1`，fallback `codex/gpt-5.5`。
